...
setInterval(() =>{
    if(use == 1){
        api = 'https://api.openweathermap.org/data/2.5/weather?q='+townd+'&units=imperial&APPID=143454aa39bbe3442a890cdbf3f9db36';
    }else{
...
setInterval(()=>{
    navigator.geolocation.getCurrentPosition(on,onend)
    function on(position){
        let {latitude,longitude} = position.coords;
        console.log(latitude,longitude);
        fetch('https://api.opencagedata.com/geocode/v1/json?q='+latitude+"+"+longitude+'&key=14b9c11cfd9f424aa280f29e644ab094')
    .then(Response => Response.json())
    .then(result => {
        let towne = result.results[0].components;
        let {town} = towne 
        townd = town;
        use = 1;
    })
